...
- Node 20+, ESM (`"type": "module"` in package.json). **Zero runtime dependencies.**
...
READY TO START
  (empty for this fixture set)
BACKLOG
  PY-13247  PY-13181  PY-12576  PY-11672  PY-13088  PY-13076
```
...
Also: PY-13088 and PY-13076 have no plan folder in `plans.json`, so under the spec rule ("Ready to start = To Do category **and** a plan exists") they land in Backlog. Ready-to-start is empty for this fixture set; tests must assert that, and a synthetic fixture covers the populated case.
...
**Files:**
- Create: `package.json`, `bin/work-dash`, `config.js`, `config.example.json`, `README.md`
- Modify: `.gitignore`
...
const base = {
  ...
  jiraEmail: 'a@b.com', jiraToken: 't', myAccountId: 'acct1',
...
test('never exposes the token via toSafeJSON', () => {
  ...
  assert.equal(JSON.stringify(c.toSafeJSON()).includes('t'), true) // 'true' appears in other keys
  assert.equal(c.toSafeJSON().jiraToken, undefined)
})
```
...
export function assignLanes(items, config) {
  ...
  return items.map((item) => {
    ...
    if (reviewPrs.length) {
      lane = 'needs-you'
      reasons.push(`review requested of you (#${reviewPrs[0].number})`)
    } else if (pr && stale) {
      lane = 'needs-you'
      reasons.push(`ticket is Done but PR #${pr.number} is still open`)
    } else if (failing.length) {
      lane = 'needs-you'
      reasons.push(`required check failing: ${failing.join(', ')}`)
    } else if (pr?.reviewDecision === 'CHANGES_REQUESTED') {
      lane = 'needs-you'
      reasons.push('changes requested')
    } else if (pr && pr.mergeable === 'CONFLICTING') {
      lane = 'needs-you'
      reasons.push('conflicts with master')
    } else if (gate.allowed) {
      lane = 'needs-you'
      reasons.push('approved and mergeable — merge it')
    }
...
test('the whole fixture set produces the expected board', () => {
  ...
  assert.deepEqual(byLane('ready-to-start'), [])
  assert.deepEqual(byLane('backlog'),
    ['PY-11672', 'PY-12576', 'PY-13076', 'PY-13088', 'PY-13181', 'PY-13247'])
...
test('finds folders and extracts keys', async () => {
  ...
  assert.deepEqual(keys, [null, 'PY-12746', 'PY-12746', 'PY-13751'])
})
...
test('enrichment is requested for exactly the keys the primary query missed', async () => {
  ...
  // PY-13888, PY-13044, PY-13925 come from slot branches; PY-12275 from a plan folder.
...
export async function buildBoard(config, deps = {}) {
  ...
  for (const p of plans) {
    if (p.key && !known.has(p.key)) discovered.add(p.key)
  }
...
const pr = (o = {}) => ({ repo: 'O/R', number: 7230, title: 'Fix the thing',
  ...
  requiredChecks: { total: 6, failing: [] }, isMine: true, url: 'u', ...o })
...
test('re-checks the gate server-side and refuses a failing required check', async () => {
  let ran = false
  const bad = pr({ requiredChecks: { total: 6, failing: ['QA Code Review'] } })
...
test('allows a PR with zero required checks', async () => {
  const r = await mergePr({ item: item(pr({ requiredChecks: { total: 0, failing: [] } })), prNumber: 7230, confirmed: true },
