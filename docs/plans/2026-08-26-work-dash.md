...
export async function buildBoard(config, deps = {}) {
  ...
  // Plan folders do NOT create items — docs/ holds ~27 keyed folders of mostly
  // finished historical work, and every enriched issue becomes a board item.
  // A plan folder only enriches a key that live work already put on the board.
  const live = new Set([...known, ...discovered])
  for (const p of plans) {
    if (p.key && live.has(p.key) && !known.has(p.key)) discovered.add(p.key)
  }
